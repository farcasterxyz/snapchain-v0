...
mod tests {
    ...
    fn test_trie_node_insert() {
        ...
        let r = node.insert(&db, &mut txn, vec![key], 0, nc());
        ...
        let r = node.insert(&db, &mut txn, vec![key.clone()], 0, nc());
        assert_eq!(r.unwrap()[0], true);
        assert_eq!(node.items(), 1);
        assert_eq!(node.value(), None);

        // Make sure the key exists
        let r = node.exists(&db, &key, 0, nc());
        ...
        let prev_hash = node.hash();
        let r = node.insert(&db, &mut txn, vec![key2.clone()], 0, nc());
        assert_eq!(r.unwrap()[0], true);
        ...
        let prev_hash = node.hash();
        let r = node.insert(&db, &mut txn, vec![key3.clone()], 0, nc());
        assert_eq!(r.unwrap()[0], true);
    ...
    fn test_trie_node_insert_one_byte() {
        ...
        let r = node.insert(&db, &mut txn, vec![key1.clone()], 0, nc());
        assert_eq!(r.unwrap()[0], true);
        ...
        let r = node.exists(&db, &key1, 0, nc()).unwrap();
        assert_eq!(r, true);

        let r = node.exists(&db, &key2, 0, nc()).unwrap();
        assert_eq!(r, true);

        // Make sure both delete Ok
        let r = node.delete(&db, &mut txn, vec![key1.clone()], 0, nc());
        assert_eq!(r.unwrap()[0], true);
        assert_eq!(node.items(), 1);

        let r = node.delete(&db, &mut txn, vec![key2.clone()], 0, nc());
        assert_eq!(r.unwrap()[0], true);
        assert_eq!(node.items(), 0);
    ...
    fn test_trie_node_delete() {
        ...
        // delete the key
        let r = node.delete(&db, &mut txn, vec![key.clone()], 0, nc());
        assert_eq!(r.unwrap()[0], true);
        assert_eq!(node.items(), 0);
        assert_eq!(node.hash(), empty_hash());

        // Getting the item after it has been deleted should return false
        let r = node.exists(&db, &key, 0, nc()).unwrap();
        assert_eq!(r, false);
        ...
        for id in ids.iter() {
            let r = node
                .insert(&db, &mut txn, vec![id.clone()], 0, nc())
                .unwrap();
            assert_eq!(r[0], true);
        }
        ...
        // Remove the first id
        let r = node
            .delete(&db, &mut txn, vec![ids[0].clone()], 0, nc())
            .unwrap();
        assert_eq!(r[0], true);

        // Expect the other 2 ids to still exist
        for id in ids.iter().skip(1) {
            let r = node.exists(&db, id, 0, nc()).unwrap();
            assert_eq!(r, true);
        }
        assert_eq!(node.items(), 2);
        ...
        let r = node
            .delete(&db, &mut txn, vec![ids[1].clone(), ids[2].clone()], 0, nc())
            .unwrap();
        ...
        let r = node.delete(&db, &mut txn, vec![ids[0].clone()], 0, nc());
        assert_eq!(r.unwrap()[0], true);
        ...
        let node1 = node
            .get_node_from_trie(&db, &ids[1][0..10], 0, nc())
            .unwrap();
        assert_eq!(node1.is_leaf(), true);
        assert_eq!(node1.value(), Some(ids[1].clone()));

        let node2 = node
            .get_node_from_trie(&db, &ids[2][0..10], 0, nc())
            .unwrap();
        ...
        for id in ids.iter().skip(1) {
            let r = node
                .delete(&db, &mut txn, vec![id.clone()], 0, nc())
                .unwrap();
            assert_eq!(r, [true]);
        }
    ...
    fn test_trie_node_hashes() {
        ...
        let all_values = node.get_all_values(&db, &[], nc()).unwrap();
        for id in ids.iter() {
    ...
    fn test_batch_insert_delete() {
        ...
        let mut txn = RocksDbTransactionBatch::new();
        let r = node.insert(&db, &mut txn, ids.clone(), 0, nc()).unwrap();
        ...
        let mut txn = RocksDbTransactionBatch::new();
        let r = node
            .insert(&db, &mut txn, new_ids.clone(), 0, nc())
            .unwrap();
        ...
        let mut txn = RocksDbTransactionBatch::new();
        let r = node
            .delete(&db, &mut txn, vec![new_ids[0].clone()], 0, nc())
            .unwrap();
        assert_eq!(r, [true]);

        // Make sure that the value is no longer there
        assert_eq!(node.exists(&db, &ids[0], 0, nc()).unwrap(), false);
        ...
        let mut txn = RocksDbTransactionBatch::new();
        let r = node.delete(&db, &mut txn, ids.clone(), 0, nc()).unwrap();
        assert_eq!(r, [false, true, true]);

        // Make sure that all the values are no longer there
        for id in ids.iter() {
            assert_eq!(node.exists(&db, id, 0, nc()).unwrap(), false);
        }
        ...
        let mut txn = RocksDbTransactionBatch::new();
        let r = node
            .delete(&db, &mut txn, new_ids.clone(), 0, nc())
            .unwrap();
        assert_eq!(r, [false, false, false, true]);

        // Make sure that the last value is no longer there
        assert_eq!(node.exists(&db, &new_ids[3], 0, nc()).unwrap(), false);
    ...
    fn test_random_batch_insert() {
        ...
        let mut txn = RocksDbTransactionBatch::new();
        let r = node.insert(&db, &mut txn, keys.clone(), 0, nc()).unwrap();
        ...
        assert_eq!(
            keys.iter()
                .all(|key| node.exists(&db, key, 0, nc()).unwrap()),
            true
        );
        ...
        for chunk in keys.chunks(100) {
            let mut txn = RocksDbTransactionBatch::new();
            let r = node.insert(&db, &mut txn, chunk.to_vec(), 0, nc()).unwrap();
        ...
        for key in keys.iter() {
            let mut txn = RocksDbTransactionBatch::new();
            let r = node
                .insert(&db, &mut txn, vec![key.clone()], 0, nc())
                .unwrap();
            assert_eq!(r.len(), 1);
            assert_eq!(r[0], true);
            db.commit(txn).unwrap();
    ...
    fn test_random_batch_delete() {
        ...
        let mut txn = RocksDbTransactionBatch::new();
        let r = node.delete(&db, &mut txn, keys.clone(), 0, nc()).unwrap();
        assert_eq!(r.len(), keys.len());
        assert_eq!(r.iter().all(|x| *x), true);
        db.commit(txn).unwrap();
        assert_eq!(node.items(), 0);
        ...
        let mut node = TrieNode::new();
        let mut txn = RocksDbTransactionBatch::new();
        node.insert(&db, &mut txn, keys.clone(), 0, nc()).unwrap();
        db.commit(txn).unwrap();
        assert_eq!(node.items(), keys.len());
        ...
        for (i, chunk) in keys.chunks(100).enumerate() {
            let mut txn = RocksDbTransactionBatch::new();
            let r = node.delete(&db, &mut txn, chunk.to_vec(), 0, nc()).unwrap();
            assert_eq!(r.len(), chunk.len());
            assert_eq!(r.iter().all(|x| *x), true);
            db.commit(txn).unwrap();
        ...
        for (i, key) in keys.iter().enumerate() {
            let mut txn = RocksDbTransactionBatch::new();
            let r = node
                .delete(&db, &mut txn, vec![key.clone()], 0, nc())
                .unwrap();
            assert_eq!(r, [true]);
            db.commit(txn).unwrap();
        ...
        let mut txn = RocksDbTransactionBatch::new();
        let r = node
            .delete(&db, &mut txn, keys[0..500].to_vec(), 0, nc())
            .unwrap();
        assert_eq!(r.len(), 500);
        assert_eq!(r.iter().all(|x| *x), true);
        db.commit(txn).unwrap();
        ...
        let r = node.delete(&db, &mut txn, keys_reversed, 0, nc()).unwrap();
        assert_eq!(r.len(), 500);
        assert_eq!(r.iter().all(|x| *x), true);
        db.commit(txn).unwrap();
        assert_eq!(node.items(), 500);
